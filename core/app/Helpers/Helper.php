...
function send_email2($to, $name, $subject, $message) {
    $set=Settings::first();
    $mlogo=Logo::first();
    $from=$set->email;
    $site=$set->site_name;
    ...
    $logo=url('/').'/asset/'.$mlogo->image_link;
    $data=array('name'=>$name,'subject'=>$subject,'content'=>$message,'website'=>$set->site_name,'phone'=>$phone,'details'=>$details,'email'=>$email,'logo'=>$logo);
    Mail::send(['html' => 'emails/mail2'], $data, function($message) use($name, $to, $subject, $from, $site) {
    $message->to($to, $name);
    $message->subject($subject);
    $message->from($from, $site);
    });
 ...
 function send_paymentlinkreceipt($ref, $type, $token) {
    ...
    $logo=url('/').'/asset/'.$mlogo->image_link;
    $receiver_subject=$amount. ' received for donation link '. $link->name.'.';
    $sender_subject=$amount. ' paid to donation link '. $link->name.'.';
    $sender_text='Payment to donation link '.$link->name.' for '. $receiver->business_name .' was successful.';
    ...
    if($dd->sender_id==null){
        ...
        $receiver_text='A payment from '.$sender_name.' was successfully received for donation link '.$link->name.'.';
    }
 ...
 function send_transferreceipt($ref) {
    ...
    $charge=$currency->name.' '.number_format((float)$link->charge,2, '.', '');
    $logo=url('/').'/asset/'.$mlogo->image_link;
    $receiver_subject=$amount. ' received from '.$dd->first_name.' '.$dd->last_name.'.';
    $sender_subject=$amount. ' transferred successfully to '.$receiver->first_name.' '.$receiver->last_name.'.';  
    $sender_text='Payment to '.$receiver->business_name.' was successful';
    ...
    if($dd->sender_id==null){
        ...
        $xx=User::whereid($dd->sender_id)->first();
        $sender_name=$xx->first_name.' '.$xx->last_name;
        $receiver_text='A payment from '.$sender_name.' was successfully sent';
    }
 ...
 function send_invoicereceipt($ref, $type, $token) {
    ...
    $charge=$currency->name.' '.number_format((float)$dd->charge,2, '.', '');
    $logo=url('/').'/asset/'.$mlogo->image_link;
    $receiver_subject=$amount. ' received for invoice '.$link->invoice_no.' successfully.';
    $sender_subject=$amount. ' paid for invoice '.$link->invoice_no.' successfully.';   
    $sender_text='Payment to invoice '.$link->invoice_no.' sent by '. $dd->first_name.' '.$dd->last_name .' for '. $receiver->business_name.' was successful';
    ...
    if($dd->sender_id==null){
        ...
        $xx=User::whereid($dd->sender_id)->first();
        $sender_name=$xx->first_name.' '.$xx->last_name;
        $receiver_text=$amount. ' received from '.$sender_name.' for invoice '.$link->invoice_no.'.';
    }
 ...
 function new_subscription($ref, $type, $token) {
    ...
    $logo=url('/').'/asset/'.$mlogo->image_link;
    $receiver_subject='Congrats, '.$sender_name.' have subscribed to '. $plan_name .'.';  
    $sender_subject='Congrats, Successfully Subscribed to '. $plan_name .'.';   
    $sender_text='Payment to subscription plan '. $plan_name .' offered by '. $receiver_name .' was successful';
    $receiver_text='A user '.$sender_name.' have successfully subscribed for your subscription plan '.$plan_name.'.';
 ...
 function send_invoice($ref) {
    ...
    $logo=url('/').'/asset/'.$mlogo->image_link;
    $sender_subject='New Payment Invoice #'.$link->invoice_no.' received from '.$receiver_name.'.';   
    $sender_text='Invoice for '.$link->item.' will be due by '.date("h:i:A j, M Y", strtotime($link->due_date));
    
    $receiver_subject='New Payment Invoice #'.$link->invoice_no.' created successfully.';    
    $receiver_text='New Payment Invoice of '.$amount.' has been created. Thanks for working with us.';
    ...
    $data=array(
        ...
        'receiver_subject'=>$receiver_subject,
        'receiver_name'=>$receiver_name,
        'receiver_name'=>$link->customer_name,
        'website'=>$set->site_name,
        'sender_text'=>$sender_text,
        'payment_link'=>$payment_link,
        'quantity'=>$quantity,
    ...
    Mail::send(['html' => 'emails/invoice/invoice_receive'], $data, function($s_message) use($receiver_name, $receiver_email,$receiver_subject, $from, $site) {
        $s_message->to($receiver_email)->subject($receiver_subject)->from($from, $site);});
 ...
 function send_invoice2($token) {
    $link=Transactions::whereref_id($token)->first();//DB::table('transactions')-where('id',$ref)->first();//Invoice::whereref_id($ref)->first();
   // $link->sent = 1;
    //$link->save();
    ...
    $ref_id=$link->ref_id;
   // $r_t=$link->charge;
    $r_t=$currency->name.' '.number_format((float)$link->charge, 2, '.', '');

    $total=$currency->name.' '.number_format((float)$link->amount,2, '.', '');
    $amount=$currency->name.' '.number_format((float)$link->amount-$link->charge,2, '.', '');
    //$tax=$currency->name.' '.number_format((float)$link->amount*1*$r_t/100, 2, '.', '');

    $logo=url('/').'/asset/'.$mlogo->image_link;
    $sender_subject='Payment Invoice #'.$link->invoice_no;   
    $sender_text='Invoice for add fund '.date("h:i:A j, M Y", strtotime($link->created_at));
    ...
    Mail::send(['html' => 'emails/invoice/invoice'], $data, function($s_message) use($sender_email, $sender_subject, $from, $site) {
    $s_message->to($sender_email)->subject($sender_subject)->from($from, $site);});
 }
 
 function send_invoice_on_cardplan($token='',$plan_id='') {
    $link=Transactions::whereref_id($token)->first();
    //$planDetails = DB::table('virtual_cards_orders')->where('id',$plan_id)->first();//Invoice::whereref_id($ref)->first();
   // $link->sent = 1;
    //$link->save();
    $user=User::whereid($link->receiver_id)->first();
    $currency=Currency::whereStatus(1)->first();
    ...
    $ref_id=$link->ref_id;
   // $r_t=$link->charge;
    $r_t=$currency->name.' '.number_format((float)0, 2, '.', '');
    ...
    Mail::send(['html' => 'emails/invoice/cardplan'], $data, function($s_message) use($sender_email, $sender_subject, $from, $site) {
    $s_message->to($sender_email)->subject($sender_subject)->from($from, $site);});
 }
 ...
 function send_request($ref) {
    ...
    $logo=url('/').'/asset/'.$mlogo->image_link;
    $sender_subject=$amount.' requested by '. $user->first_name.' '.$user->last_name.'.';   
    ...
    $data=array(
        ...
        'receiver_name'=>$to->first_name.' '.$to->last_name,
        'payment_link'=>$payment_link,
        'amount'=>$amount,
        'charge'=>$charge,
        'reference'=>$ref,
...
if (! function_exists('boomtime'))
{
 ...
 function send_singlepaymentlinkreceipt($ref) {
    $link=Paymentlink::whereref_id($ref)->first();
    $receiver=User::whereid($link->user_id)->first();
    $currency=Currency::whereStatus(1)->first();
    $set=Settings::first();
    ...
    $logo=url('/').'/asset/'.$mlogo->image_link;
    $receiver_subject='New successful transaction'; 
    $receiver_text='Payment was successful';
    ...
    Mail::send(['html' => 'emails/payment_links/single_receive'], $data, function($r_message) use($receiver_name, $receiver_email, $receiver_subject, $from, $site) {
        $r_message->to($receiver_email, $receiver_name)->subject($receiver_subject)->from($from, $site);});  
 } 

 function send_plan($ref) {
    $link=Plans::whereref_id($ref)->first();
    $receiver=User::whereid($link->user_id)->first();
    $currency=Currency::whereStatus(1)->first();
    $set=Settings::first();
    ...
    $logo=url('/').'/asset/'.$mlogo->image_link;
    $receiver_subject='New plan created successful.';
    $receiver_text='A plan of amount '.$amount.' was successfully created.';  
    ...
    Mail::send(['html' => 'emails/subscription/receiver/plan'], $data, function($r_message) use($receiver_name, $receiver_email, $receiver_subject, $from, $site) {
        $r_message->to($receiver_email, $receiver_name)->subject($receiver_subject)->from($from, $site);});  
 }
 
  function send_product($ref) {
    $link=Product::whereref_id($ref)->first();
    $receiver=User::whereid($link->user_id)->first();
    $currency=Currency::whereStatus(1)->first();
    $set=Settings::first();
    ...
    $product_quant=$link->quantity;

    $logo=url('/').'/asset/'.$mlogo->image_link;
    $receiver_subject='New product created successfully.';
    $receiver_text='A product of amount '.$amount.' was successfully created.';  
    ...
    Mail::send(['html' => 'emails/product/newproduct'], $data, function($r_message) use($receiver_name, $receiver_email, $receiver_subject, $from, $site) {
        $r_message->to($receiver_email, $receiver_name)->subject($receiver_subject)->from($from, $site);});  
 }