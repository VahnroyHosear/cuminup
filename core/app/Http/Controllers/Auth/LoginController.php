...
class LoginController extends Controller
{
    ...
    */
    ...
    public function submitlogin(Request $request)
    {
        ...
        if (Auth::guard('user')->attempt([
            ...
            if($user->fa_status==1){
                ...
                if(Session::has('oldLink')){
                    ...
                    {
                      if($_SERVER['REMOTE_ADDR'] == $user->last_login_ip)
                         {
                            return redirect()->route('user.newdashboard'); 
                         } else {
                             return redirect()->route('user.login_with_otp');
                         }
                    } else {
                         if($_SERVER['REMOTE_ADDR'] == $user->last_login_ip)
                         {
            ...
            */
            ...
            if(Session::get('wrong_attempt_count') >= '3')
            {
               ...
               if($userdata)
               {
            	...
            	send_email($request->email, $userdata->first_name, 'Account Suspended', 'Dear '.$userdata->first_name.',<br> Your account has been suspended due to multiple login attempt or suspiciously activities, for any account related kindly <a href="mailto:support@cuminup.com">Get in touch</a> with us. <br><br>Please contact our support team- support@cuminup.com');
                $user=User::find($userdata->id);
                $user->status=1;
                $user->save();
                return back()->with('alert', 'Oops! Your account has been suspended!')->withInput($request->only('email', 'remember'));   
               }
        ...
        if(empty($emailCheck))
        {
          return back()->with('alert', 'Oops! You have entered invalid email!')->withInput($request->only('email', 'remember')); 
          }elseif(DB::table('users')->where(['email'=>$emailCheck->email,'password'=>Hash::make($request->password)])->first())
          {
              return back()->with('alert', 'Oops! You have entered invalid password!')->withInput($request->only('email', 'remember')); 
          }
          else {
            return back()->with('alert', 'Oops! You have entered invalid password!')->withInput($request->only('email', 'remember')); 
         }
    ...
    public function submitloginphone(Request $request)
    {
        ...
        if (Auth::guard('user')->attempt([
            ...
            $emailCheck = DB::table('users')->where('phone',$request->phone)->first();
        
        if(empty($emailCheck))
        {
          return back()->with('alert', 'Oops! You have entered invalid phone!')->withInput($request->only('email', 'remember')); 
          }elseif(DB::table('users')->where(['phone'=>$emailCheck->phone,'password'=>Hash::make($request->password)])->first())
          {
              return back()->with('alert', 'Oops! You have entered invalid password!')->withInput($request->only('email', 'remember')); 
          }
          else {
