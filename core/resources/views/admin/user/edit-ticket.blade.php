              ...
              @if($df->status==1)
                ...
                @elseif($df->status==0)
                  <div class="timeline-block">
                      ...
                      <small class="text-xs">{{date("Y/m/d h:i:A", strtotime($df->created_at))}}</small>
                      <h5 class="mt-3 mb-0">{{$df->reply}}</h5>
                      <p class="text-sm mt-1 mb-0">@if($df->staff_id==1) {{__('Support - GetVirtualCard')}} @else {{$df->staff['first_name'].' '.$df->staff['last_name']}} - <span class="badge badge-pill badge-success">Staff</span> @endif</p>
                      </div>
                  </div>
                @endif
            @endforeach
