    ...
    <div class="content-wrapper">
        <div class="card">
            <div class="card-header header-elements-inline">
                <h3 class="mb-0">{{__('All Customers')}}</h3>
            </div>
                ...
                <table class="table table-flush" id="datatable-buttons_3434">
                    ...
                    @foreach($users as $k=>$val)
                        <tr>
                            ...
                            <!--td class="shorting_1"><span style="display:none;">{{strtotime($val->created_at)}}</span>{{++$k}}.</td-->
                            <td class="shorting_1"><span style="display:none;">{{strtotime($val->created_at)}}</span><a href="{{route('user.manage', ['id' => $val->id])}}" class="dropdown-item" style="color:blue;">{{$val->first_name.' '.$val->last_name}}</a></td>
                            <td>{{$val->business_name}}</td>
                            <td style="text-transform: capitalize;">{{ $val->account_type }}</td>
                            <td>{{$val->email}}</td>
                            <td>{{'+'}}{{$val->prefix}}{{$val->phone}}</td>
                            <td>
                                @if($val->account_type == 'standard')
                                    {{$val->verify_ssn}}
                                @else  
                                    {{$val->verify_ein}}
                                @endif
