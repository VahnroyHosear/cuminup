  ...
  <nav class="sidenav navbar navbar-vertical fixed-left navbar-expand-xs navbar-dark bg-cyan" id="sidenav-main">
    <div class="scrollbar-inner">
      ...
      <div class="navbar-inner">
        <!-- Collapse -->
        @if(Auth::guard('admin')->user()->username == 'seovirtualcard')
            <div class="collapse navbar-collapse" id="sidenav-collapse-main">
                ...
                <hr class="my-3">
                <h6 class="navbar-heading p-0 text-muted">{{__('SEO Details')}}</h6>
        ...
        @else
            <div class="collapse navbar-collapse" id="sidenav-collapse-main">
              ...
              <ul class="navbar-nav mb-md-3"> 
                ...
                <li class="nav-item">
                  <a class="nav-link @if(route('admin.enterprise_pending')==url()->current()) active @endif" href="{{route('admin.enterprise_pending')}}">
                    <i class="fa fa-user"></i>
                    <span class="nav-link-text">{{__('Enterprise Pending Request')}}</span>
                  </a>
                </li>
                @endif         
                @if($admin->id==1)    
              ...
              <li class="nav-item">
                  <a class="nav-link @if(route('admin.virtual_cards')==url()->current() || route('admin.virtual_cards_transactions')==url()->current() || route('admin.vcard_orders')==url()->current() || route('admin.vcard_orders')==url()->current()) active @endif" href="#card7676" data-toggle="collapse" role="button" aria-expanded="false" aria-controls="navbar-examples">
                  ...
                  <div class="collapse @if(route('admin.virtual_cards')==url()->current() || route('admin.virtual_cards_transactions')==url()->current() || route('admin.vcard_orders')==url()->current()) show @endif" id="card7676">
                    <ul class="nav nav-sm flex-column">
                      <li class="nav-item @if(route('admin.virtual_cards')==url()->current()) active @endif"><a href="{{route('admin.virtual_cards')}}" class="nav-link">{{__('Virtual Card Users List')}}</a></li>
                     <li class="nav-item @if(route('admin.vcard_orders')==url()->current()) active @endif"><a href="{{route('admin.vcard_orders')}}" class="nav-link">{{__('vCard Orders List')}}</a></li>
                      ...
                      <li class="nav-item @if(route('admin.physical_virtual_card')==url()->current()) active @endif"><a href="{{route('admin.physical_virtual_card')}}" class="nav-link">{{__('Physical Card Users')}}</a></li>
                ...
                <li class="nav-item">
                  <a class="nav-link @if(route('admin.subscription_list')==url()->current() || route('admin.subscription_list')==url()->current() || route('admin.deposit.log')==url()->current()) active @endif" href="#card77778" data-toggle="collapse" role="button" aria-expanded="false" aria-controls="navbar-examples">
                    <i class="fa fa-inr"></i>
                    <span class="nav-link-text">{{__('Virtual Card Setting')}}
                      @if(count($pdeposit)>0 || count($pbank)>0)
                      <!--span class="badge badge-sm badge-circle badge-floating badge-danger border-white">{{count($pdeposit) + count($pbank)}}</span-->
                      @endif
                    </span>
                  </a>
                  <div class="collapse @if(route('admin.subscription_list')==url()->current() || route('admin.subscription_list')==url()->current() || route('admin.card_type_list')==url()->current() || route('admin.card_design_list')==url()->current()) show @endif" id="card77778">
                    <ul class="nav nav-sm flex-column">
                      <li class="nav-item @if(route('admin.subscription_list')==url()->current()) active @endif"><a href="{{route('admin.subscription_list')}}" class="nav-link">{{__('Subscription Type')}}</a></li>
              ...
              <ul class="navbar-nav mb-md-3"> 
                ...
                @if($admin->id==1)
                <li class="nav-item">
                  <a class="nav-link  @if(route('homepage')==url()->current() || route('admin.service')==url()->current() || route('admin.brand')==url()->current() || route('admin.logo')==url()->current() || route('admin.review')==url()->current() || route('admin.page')==url()->current() || route('admin.faq')==url()->current() || route('admin.currency')==url()->current() || route('admin.terms')==url()->current() || route('privacy-policy')==url()->current() || route('about-us')==url()->current() || route('social-links')==url()->current() || route('e-sign')==url()->current()) active @endif" href="#xx" data-toggle="collapse" role="button" aria-expanded="false" aria-controls="navbar-examples">
                    <i class="fa fa-desktop"></i>
                    <span class="nav-link-text">{{__('Website design')}}</span>
                  </a>
                  <div class="collapse @if(route('homepage')==url()->current() || route('admin.service')==url()->current() || route('admin.brand')==url()->current() || route('admin.logo')==url()->current() || route('admin.review')==url()->current() || route('admin.page')==url()->current() || route('admin.faq')==url()->current() || route('admin.currency')==url()->current() || route('admin.terms')==url()->current() || route('privacy-policy')==url()->current() || route('about-us')==url()->current() || route('social-links')==url()->current() || route('e-sign')==url()->current()) show @endif " id="xx">
                    <ul class="nav nav-sm flex-column">
                        <li class="nav-item @if(route('homepage')==url()->current()) active @endif"><a href="{{route('homepage')}}" class="nav-link">{{__('Homepage')}}</a></li>
                        <li class="nav-item @if(route('admin.brand')==url()->current()) active @endif"><a href="{{route('admin.brand')}}" class="nav-link">{{__('Brands')}}</a></li>	
                        <li class="nav-item @if(route('admin.logo')==url()->current()) active @endif"><a href="{{route('admin.logo')}}" class="nav-link">{{__('Logo & Favicon')}}</a></li>	
                        ...
                        <li class="nav-item @if(route('admin.page')==url()->current()) active @endif"><a href="{{route('admin.page')}}" class="nav-link">{{__('Webpages')}}</a></li>
                        <li class="nav-item @if(route('admin.faq')==url()->current()) active @endif"><a href="{{route('admin.faq')}}" class="nav-link">{{__('FAQs')}}</a></li>
                        <li class="nav-item @if(route('admin.country')==url()->current()) active @endif"><a href="{{route('admin.country')}}" class="nav-link">{{__('Country')}}</a></li>
                        <li class="nav-item @if(route('admin.currency')==url()->current()) active @endif"><a href="{{route('admin.currency')}}" class="nav-link">{{__('Currency')}}</a></li>
                        <li class="nav-item @if(route('admin.terms')==url()->current()) active @endif"><a href="{{route('admin.terms')}}" class="nav-link">{{__('Terms & Condition')}}</a></li>
                        <li class="nav-item @if(route('privacy-policy')==url()->current()) active @endif"><a href="{{route('privacy-policy')}}" class="nav-link">{{__('Privacy policy')}}</a></li>
                        <li class="nav-item @if(route('about-us')==url()->current()) active @endif"><a href="{{route('about-us')}}" class="nav-link">{{__('About us')}}</a></li>
                        <li class="nav-item @if(route('social-links')==url()->current()) active @endif"><a href="{{route('social-links')}}" class="nav-link">{{__('Social Links')}}</a></li>   
                        <li class="nav-item @if(route('e-sign')==url()->current()) active @endif"><a href="{{route('e-sign')}}" class="nav-link">{{__('e-Sign Consent')}}</a></li>   
                ...
                <li class="nav-item">
                  <a class="nav-link @if(route('admin.seo')==url()->current()) active @endif" href="{{route('admin.seo')}}">
      ...
      <div class="container-fluid">
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          ...
          <ul class="navbar-nav align-items-center ml-auto ml-md-0">
             ...
             <li class="nav-item dropdown"> <a href="{{url('admin/ticket')}}"><i class="fa fa-question-circle" style="font-size:26px;cursor:poniter" aria-hidden="true"></i></a></li>
            <li class="nav-item dropdown">
                
               
                <a class="nav-link dropdown-toggle" data-toggle="dropdown" href="#"><i class="fa fa-bell" style="color:#256377"></i> <span class="badge badge-sm badge-circle badge-floating badge-danger border-white"> {{count($notification)}}</span></a>
                @if(count($countkyc)>0)
                <!--div class="dropdown-menu" style="height: 200px;    overflow-y: scroll;">
                    @foreach($countkyc as $kyc)
                    <a class="dropdown-item" href="{{route('user.manage', ['id' => $kyc->id])}}" target="_blank">{{$kyc->first_name}} {{$kyc->last_name}} <br><span style="color:green">Requested for enterprise account.</span>  </a>
                    @endforeach
                </div--> 
                @endif 
                ...
                <div class="dropdown-menu" style="height: 200px;    overflow-y: scroll;">
                    @foreach($notification as $notify_details)
                     ...
                     {{$notify_details->receiver_description}}  
                    </br>
