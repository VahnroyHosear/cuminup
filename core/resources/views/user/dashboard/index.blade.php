    ...
    <div class="col-lg-4">
        <div class="card bg-white shadow">
          ...
          @if(Auth::user()->user_type == 1)
           <a href="{{route('user.virtualcard')}}" class="btn btn-success btn-sm"><i class="fa fa-arrow-right"></i> {{__('Manage Virtual Cards')}}</a>
              ...
              <!--<center><a href="{{route('user.upgrade')}}"><h3 class="mr-3 upgrade-premium mb-5"><i class="fas fa-crown"></i> <span class="mobile-view">Upgrade to Business</span></h3></a></center>-->
                ...
                <div class="alert alert-error alert-dismissible" style="height:50px;background-color: #dd4b39;color: #ffffff;">
    				...
    				<span class="pull-right">
    					<a href="{{route('user.doc-verification')}}" class="btn bg-navy" style="display:unset !important;background-color: #001f3f ;color: #ffffff;"><i class="fa fa-rocket"></i>  Upgrade Now</a>
    				</span>
                </div>
                <!--@endif-->
          </div>
      ...
      <div class="col-lg-4">
        <div class="card bg-white shadow">
          <div class="card-body">
            ...
            @if(Auth::user()->user_type == 1)
            <a class="btn btn-disabled btn-sm" type="button" data-toggle="modal" data-target=".bd-example-modal-sm"><i class="fa fa-arrow-right"></i> {{__('Create Invoice')}}</a>
            @else
            <a href="{{url('user/invoice')}}" class="btn btn-success btn-sm"><i class="fa fa-arrow-right"></i> {{__('Create Invoice')}}</a>
            @endif
        ...
        <div class="card bg-white shadow">
          <div class="card-body">
            ...
            <p class="card-text text-sm">{{__('Sending payment link for single charge is now possible. Single charge is a one time payment for any needed service or product.')}}</p>
             @if(Auth::user()->user_type == 1)
            <a class="btn btn-disabled btn-sm" type="button" data-toggle="modal" data-target=".bd-example-modal-sm"><i class="fa fa-arrow-right"></i> {{__('Payment Links')}}</a>
            @else
       ...
       <div class="col-lg-4">
        <div class="card bg-white shadow">
          <div class="card-body">
            <div class="row align-items-center">
              <div class="col-12">
                <h3 class="card-title mb-3">{{__('Sell Online')}}</h3>
              </div>
            </div>
            <p class="card-text text-sm">{{__('Digital and any product can now be sold easily. Create & share your products to clients')}}</p>
         @if(Auth::user()->user_type == 1)
         <a class="btn btn-disabled btn-sm" type="button" data-toggle="modal" data-target=".bd-example-modal-sm"><i class="fa fa-arrow-right"></i> {{__('Sell Product')}}</a>
         @else
      ...
      <div class="col-lg-4">        
        <div class="card bg-white shadow">
          <div class="card-body">
            <div class="row align-items-center">
              <div class="col-12">
                <h3 class="card-title mb-3">{{__('Donation')}}</h3>
              </div>
            </div>
            <p class="card-text text-sm">{{__('You can now create a donation to get funds for any project')}}</p>
            @if(Auth::user()->user_type == 1)
        ...
        @if($set->merchant==1)
          <div class="card bg-white shadow">
            <div class="card-body">
              <div class="row align-items-center">
                <div class="col-12">
                  <h3 class="card-title mb-3">{{__('Payment URLs')}}</h3>
                </div>
              </div>
              <p class="card-text text-sm">Receiving money on your website is now easy with simple integeration at a fee of {{$set->single_charge}}% per transaction</p>
              @if(Auth::user()->user_type == 1)
              ...
              @else
                <a href="{{url('user/sc-links')}}" class="btn btn-success btn-sm"><i class="fa fa-arrow-right"></i> {{__('Payment URLs')}}</a>
              @endif
            </div>
          </div>
        @endif
