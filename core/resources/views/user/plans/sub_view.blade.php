          ...
          <div class="card card-profile bg-white border-0 mb-5">
            ...
            <div class="card-body pt-7 px-5">
              <div class="row justify-content-between align-items-center mb-3">
                  <label class="col-form-label col-lg-3.5">{{__('Payable Amount')}}</label>
                @if($link->amount!=null)
                    <span class="form-text text-xl"><center>{{$currency->symbol}} {{number_format($link->amount,2)}}</center></span>
                @endif
              </div>
              <form role="form" action="{{ route('submit.plancharge')}}" method="post" id="modal-details">
                ...
                @if($link->times!=null)
                <div class="row justify-content-between align-items-center mb-3">
                    <label class="col-form-label col-lg-3.5">{{__('Auto renewal')}}</label>
                ...
                @endif
                <div class="row justify-content-between align-items-center mb-3">
                  <label class="col-form-label col-lg-3.5">{{__('Validity')}}</label>
                <div class="text-right">  
                    ...
                    <div class="modal-content">
                      <div class="modal-body p-0">
                        <div class="accordion" id="accordionExample">
                             <h3 class="mb-0" style="padding: 15px;">{{__('Pay with Available Balance')}} <button type="button" class="close" style="float:right" data-dismiss="modal">&times;</button></h3>
                              ...
                              <div class="card-body text-center">
                                @if (Auth::guard('user')->check())
                                  <form method="post" role="form" action="{{route('submit.plancharge')}}">
                                    @csrf
                                    <h4 class="mb-0">{{__('Available Balance')}}</h4>
                ...
                <div class="text-center">
                    @if (Auth::guard('user')->check())
                                  <button type="button" data-toggle="modal" data-target="#fund" class="btn btn-success my-4">{{__('Pay Now')}}</button>
                                @else
                                @php
                                Session::put('oldLink', url()->current());
                                @endphp
                                   <a href="{{route('login')}}" class="btn btn-neutral btn-sm">{{__('Login and Pay')}}</a>
                                @endif
