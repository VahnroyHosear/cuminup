  ...
  <div class="content-wrapper">
    <div class="card">
          ...
          <tbody>  
            @foreach($donation as $k=>$val)
              <tr>
                ...
                <td>@if($val->sender_id==$user->id || $val->charge==null) - @else {{$currency->symbol.number_format($val->charge,2)}} @endif</td>
                <td>{{$val->ref_id}}</td>
      ...
      <center>
           <div class="col-lg-12">
              <br>
                <p class="text-center card-text mt-5">{{__('No donation log Found!')}}</p>
               <h3>{{__('Let’s Create your first donation page')}}</h3>
             <a href="{{url('user/dp-links')}}" class="btn btn-sm btn-neutral"><i class="fa fa-plus"></i> {{__('Get Started')}}</a>
